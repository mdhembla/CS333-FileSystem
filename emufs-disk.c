...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	met = (struct metadata_t*)buf;
	if(superblock->fs_number==1){
		decrypt(mounts[mount_point].key,(char*)&met->inodes[inodenum],sizeof(struct inode_t));
	}
	
	memcpy(inodeptr, &met->inodes[inodenum], sizeof(struct inode_t));
}
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	int id = inodenum;
	if(inodenum < 16){
		readblock(mounts[mount_point].device_fd, 1, buffer);
	} 
	else {
		readblock(mounts[mount_point].device_fd, 2, buffer);
		inodenum -= 16;
	}

	met = (struct metadata_t*)buffer;
	if(superblock->fs_number==1){
		decrypt(mounts[mount_point].key,(char*)&met->inodes[inodenum],sizeof(struct inode_t));
	}
	memcpy(&met->inodes[inodenum], inodeptr, sizeof(struct inode_t));
	if(superblock->fs_number==1){
		encrypt(mounts[mount_point].key,(char*)&met->inodes[inodenum],sizeof(struct inode_t));
	}

	if(id < 16){
		writeblock(mounts[mount_point].device_fd, 1, (char*)met);
	} 
	else {
		writeblock(mounts[mount_point].device_fd, 2, (char*)met);
	}
...
void read_datablock(int mount_point, int blocknum, char *buf){
	...
	*/
	...
	if(mounts[mount_point].fs_number == 1) decrypt(mounts[mount_point].key, buf, BLOCKSIZE);
}
...
void write_datablock(int mount_point, int blocknum, char *buf){
	...
	*/
	if(mounts[mount_point].fs_number == 1) encrypt(mounts[mount_point].key, buf, BLOCKSIZE);
